...
export class CovidComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.listLastestByCountry = combineLatest([this.isoUpdater]).pipe(
      ...
      tap(res => {
        this.map.panTo(new L.LatLng((res as any)[0]?.location.lat || 21.030653, (res as any)[0]?.location.lng || 105.847130));
        const newMarker = L.marker([(res as any)[0]?.location?.lat || 21.030653, (res as any)[0]?.location?.lng || 105.847130], this.markerIcon);
